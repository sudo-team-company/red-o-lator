...
void run_s_cbranch_g_fork(WfStateSOP2& state, Wavefront* wavefront) {
...
void run_s_lshl_n_add_u32(WfStateSOP2& state, size_t n) {
...
void run_s_mul_hi_i32(WfStateSOP2& state) {
    state.SDST =
        (static_cast<int64_t>(state.SSRC0) * static_cast<int32_t>(state.SSRC1)) >> 32;
}
...
void run_s_nand_b32(WfStateSOP2& state) {
    state.SDST =
        ~(static_cast<uint32_t>(state.SSRC0) & static_cast<uint32_t>(state.SSRC1));
    state.SCC = state.SDST != 0;
}
...
void run_sop2(const Instruction& instr, Wavefront* wf) {
    ...
    switch (instr.get_key()) {
        ...
        case S_XOR_B64:
            ...
            break;
        default:
            UNSUPPORTED_INSTRUCTION("SOP2", get_mnemonic(instr.get_key()));
    }
