 ...
 * }
    ...
    TEST_CASE("c = a + b") {
        ...
        KernelConfig kernelConfig = KernelParser::parseKernelConfig(
            1, std::vector<size_t>(), globalWorkSize, globalWorkSize, config);
 ...
 * }
    ...
    TEST_CASE("data[global_id(0)] = x + x") {
        ...
        const std::vector<std::string>& config = {
            ...
            ".arg x, \"int\", int"};
        KernelConfig kernelConfig = KernelParser::parseKernelConfig(
            1, std::vector<size_t>(), globalWorkSize, localWorkSize, config);
        kernelConfig.kernArgAddr = kernArgAddr;
        const std::vector<std::string>& instructions = {
 ...
 * }
    ...
    TEST_CASE("data[global_id(i)] = global_offset(i) + x") {
        ...
        const std::vector<std::string>& config = {
            ...
            ".arg data, \"int*\", int*, global"};
        KernelConfig kernelConfig = KernelParser::parseKernelConfig(
            3, std::vector<size_t>(), globalWorkSize, localWorkSize, config);
        kernelConfig.kernArgAddr = kernArgAddr;
        const std::vector<std::string>& instructions = {
    ...
    TEST_CASE("if_1") {
        ...
        const std::vector<std::string>& config = {
            ...
            ".arg y,    \"int\", int"};
        KernelConfig kernelConfig = KernelParser::parseKernelConfig(
            2, std::vector<size_t>(), globalWorkSize, std::vector<size_t>(), config);
        kernelConfig.kernArgAddr = kernArgAddr;
        const std::vector<std::string>& instructions = {
            "/*000000000000*/ s_load_dwordx4  s[0:3], s[4:5], 0x0",
            "/*000000000008*/ s_waitcnt       lgkmcnt(0)",
    ...
    /**
    * __kernel void weighted_sum_kernel(int n, __global float *a, __global float *b,
    __global float *s, __global float *c)
    {
    ...
    * __kernel void transpose_naive(__global float *odata, __global float* idata, int
